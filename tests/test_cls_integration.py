...
def test_lattice_resolve_basic():
    ...
    assert intensity == 1.1, f"Expected intensity 1.1, got {intensity}"
...
def test_lattice_resolve_repeat():
    ...
    assert math.isclose(intensity1, 1.1, rel_tol=1e-9), f"Expected first intensity 1.1 (within tolerance), got {intensity1}"
    
    # Second resolve (should add another cls_event)
    intensity2 = lattice_resolve(geo_a, geo_b)
    assert math.isclose(intensity2, 1.2, rel_tol=1e-9), f"Expected second intensity 1.2 (within tolerance), got {intensity2}"
    
    # Third resolve
    intensity3 = lattice_resolve(geo_a, geo_b)
    assert math.isclose(intensity3, 1.3, rel_tol=1e-9), f"Expected third intensity 1.3 (within tolerance), got {intensity3}"
...
def test_create_lattice_form():
    ...
    assert form.intensity_sum(apply_time_decay=False) == 1.1  # 0.5 + 0.5 + 0.1
...
def test_lattice_integration_flow():
    ...
    assert math.isclose(intensity1, 1.1, rel_tol=1e-9)  # Initial: 1.0 + 0.1
    assert math.isclose(intensity2, 1.2, rel_tol=1e-9)  # Second: 1.0 + 0.1 + 0.1
    assert math.isclose(form.intensity_sum(apply_time_decay=False), 1.1, rel_tol=1e-9)  # 0.5 + 0.5 + 0.1
    assert math.isclose(restored_form.intensity_sum(apply_time_decay=False), 1.1, rel_tol=1e-9)
